        	...
        	if (row != null) {
        	    ...
        	    if (row[2] != null) {
        	       ...
        	       BigDecimal totalFee = feeCalculatorService.calculateTotalFee(backendAmount);
        	        appointmentMap.put("amount", totalFee.toString());  // store temporarily
        	    } else {
	...
	public static HashMap<String, String> setAppointmentPaid(HashMap<String, Object> appointmentMap) {
		...
		try {
			...
			Object[] row = (Object[]) session
					.createNativeQuery("SELECT cc.currency_name, " + " (SELECT sc.Fee FROM ServiceContractDetails sc "
							+ "   WHERE sc.UserID = ( " + "       SELECT r2.registration_id FROM registration r2 "
							+ "       WHERE r2.DocID = :docId LIMIT 1) " + "   LIMIT 1) AS Fee "
							+ "FROM registration r " + "LEFT JOIN countries_currencies cc "
							+ "   ON UPPER(cc.country_code) = UPPER(r.country_code) "
							+ "WHERE r.registration_id = :userId")
					.setParameter("userId", userId).setParameter("docId", docId).uniqueResult();
			...
			AvailabilitySchedule doctorAvailability = session.get(AvailabilitySchedule.class,
					(Integer) appointmentMap.get("docID"));
			...
			HashMap<String, String> payRes = PaymentGatewayDaoImpl.setPayment(appointmentMap,
