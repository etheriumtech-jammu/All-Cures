...
public Doctors(String docJsonStr){
	/*if(docsSplit != null){
		String [] docProfile = docsSplit.split(Constant.DefaultStringSplitter);
		JSONObject docJson = new JSONObject();
		String key, value;
		for(int i=0; i < docProfile.length - 1; i++){
			String []keyVal;
			keyVal = docProfile[i].split(":");
			Constant.log(docProfile[i], 1);
			key = keyVal[0];
			Constant.log(key, 1);
			value = keyVal[1];
			Constant.log(value, 1);
			docJson.append(key, value);
		}*/
		/* 
		//JSON based implementation is better
		 
		//Json of the Doctor of the format: docid:1|prefix:Mr.|
		//Need to change the Seperator to "|"; Maybe do a replaceAll of the DefaultStringSplitter with (DefaultJsonSplitter) "," here
		//or set the Json Objects Seperator to "|"
		...
		*/
		...
		//TODO: Handle Could be Multiple Values; Need this to be a JSON Array
		if(docJson.get(Constant.INSURANCE_ACCEPT) != null && !JSONObject.NULL.equals(docJson.get(Constant.INSURANCE_ACCEPT)))
			this.setInsurance_accept((Boolean) docJson.get(Constant.INSURANCE_ACCEPT));
		//TODO: Handle Could be Multiple Values; Need this to be a JSON Array
		if(docJson.get(Constant.HOSPITAL_AFFLIATED) != null && !JSONObject.NULL.equals(docJson.get(Constant.HOSPITAL_AFFLIATED)))
			this.setHospital_affliated((String) docJson.get(Constant.HOSPITAL_AFFLIATED));
		//TODO: Handle Could be Multiple Values; Need this to be a JSON Array
		if(docJson.get(Constant.BOARD_CERTIFICATION) != null && !JSONObject.NULL.equals(docJson.get(Constant.BOARD_CERTIFICATION)))
		...
		//TODO: Handle Could be Multiple Values; Need this to be a JSON Array
		if(docJson.get(Constant.AVAILIBITY_FOR_APPOINTMENT) != null && !JSONObject.NULL.equals(docJson.get(Constant.AVAILIBITY_FOR_APPOINTMENT))){
			try{
		...
		if(docJson.get(Constant.DOCTOR_LOCATION) != null && !JSONObject.NULL.equals(docJson.get(Constant.DOCTOR_LOCATION)))
		...
		if(docJson.get(Constant.OVER_ALLRATING) != null && !JSONObject.NULL.equals(docJson.get(Constant.OVER_ALLRATING)))
		...
		if(docJson.get(Constant.DOCNAME_MIDDLE) != null && !JSONObject.NULL.equals(docJson.get(Constant.DOCNAME_MIDDLE)))
...
public Doctors(Integer docid, Integer gender, String edu_training, boolean insurance_accept, String hospital_affliated,
		String board_certifcate, String membership, String awards, Integer waiting_time,
		...
		Specialties specialties, City city, States states, model.countries countries) {
	super();
	this.docid = docid;
	this.gender = gender;
